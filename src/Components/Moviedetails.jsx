...
import { detailApi } from "../Store/Actions/movieActions";
import { removedata } from "../Store/Reducers/movieSlice";
import {
  Link,
  Outlet,
  useLocation,
...
const Moviedetails = () => {
  ...
  return data ? (
    ...
    >
      ...
      <div className="flex flex-col md:flex-row lg:flex-row ml-7 md:ml-14 lg:ml-20 mt-9 rounded-md overflow-hidden ">
        <img
          className=" w-80 h-96 object-cover ml-3 md:ml-0 lg:ml-0 "
        ...
        <div className=" p-4 md:p-7 lg:p-11">
          ...
          <p className="text-xs lg:text-sm text-white flex  items-center gap-2 mt-2 min-w-fit">
            ...
            <span className=" pl-5 text-zinc-20 font-semibold">
      ...
      <div className="flex flex-col justify-center space-y-4  px-10 md:px-18 lg:px-20 md:py-7 lg:py-7 ">
        ...
        <div className="flex items-center gap-2 md:gap-4 lg:gap-5  ">
          <h1 className="text-white font-semibold min-w-fit text-sm lg:text-lg xl:text-lg ">
            Available to Buy
          </h1>
          {data.watchprovider && data.watchprovider.buy ? (
            data.watchprovider.buy.map((item, index) => (
              <img
                className="w-9 rounded-md"
                key={index}
                src={`https://image.tmdb.org/t/p/original${item.logo_path}`}
        ...
        <div className="flex items-center gap-2 md:gap-4 lg:gap-5  ">
          ...
          {data.watchprovider && data.watchprovider.rent ? (
            data.watchprovider.rent.map((item, index) => (
              <img
                className="w-9 rounded-md"
                key={index}
                src={`https://image.tmdb.org/t/p/original${item.logo_path}`}
      ...
      <div className="flex items-center overflow-auto px-3   gap-8">
        {data.recommendations && data.recommendations.length > 0 ? (
          ...
          data.similar.map((item, index) => (
            ...
            <p className="text-xl w-full font-semibold text-center py-4 text-white">Not Found !</p>
        )}
      </div>
      <Outlet />
    </div>
